...
class PatchEmbedding(nn.Layer):
    def __init__(self, image_size=224, patch_size=16, in_channels=3, embed_dim=768, dropout=0.):
        ...
        # 添加 class token，定义一个可学习的参数
        self.class_token = paddle.create_parameter(
            shape=[1, 1, embed_dim],  # shape和标准token一样,不知道batch_size多少先用1代替
            dtype='float32',
            default_initializer=nn.initializer.Constant(0)  # 初始值通常给0就行
        )

        # 添加 position embedding，定义一个可学习的参数
        self.position_embedding = paddle.create_parameter(
            shape=[1, n_patches+1, embed_dim],
            dtype='float32',
            default_initializer=nn.initializer.TruncatedNormal(std=.02)  # 用来一种初始化方式，用全零可能不太好
        )

    def forward(self, x):
        # [n, c, h, w]
        class_tokens = self.class_token.expand([x.shape[0], -1, -1])  # for batch的，现在知道batch_size
        x = self.patch_embedding(x)  # [n, embed_dim , h', w']
        x = x.flatten(2)  # [n, embed_dim, h'*w']
        x = x.transpose([0, 2, 1])  # [n, h'*w', embed_dim]
        x = paddle.concat([class_tokens, x], axis=1)  # [n, h'*w+1, embed_dim]
...
class VisualTransformer(nn.Layer):
    ...
    def forward(self, x):
        ...
        # x = x.transpose([0, 2, 1])  # [N, num_patchs, embed_dim]
        x = self.encoder(x)
        print(x.shape)
